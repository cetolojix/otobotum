...
export function WhatsAppInstanceManager({ user, profile, instances }: WhatsAppInstanceManagerProps) {
  ...
  const fetchPackageInfo = async () => {
    try {
      ...
      debugLog("Paket bilgisi alınamadı:", error)
    } finally {
      setLoading(false)
    }
  }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              ...
              <Card className="hologram-card border-0 backdrop-blur-sm shadow-xl">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">İnstanslarım</CardTitle>
                  <MessageSquare className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">{connectedInstances.length}</div>
                  <p className="text-xs text-muted-foreground">Toplam WhatsApp bot</p>
                </CardContent>
              </Card>
              ...
              <Card className="hologram-card border-0 backdrop-blur-sm shadow-xl">
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {connectedInstances.filter((i) => i.status === "connected").length}
                  </div>
                  <p className="text-xs text-muted-foreground">Şu anda bağlı</p>
                </CardContent>
              </Card>

              <Card className="hologram-card border-0 backdrop-blur-sm shadow-xl">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">İş Akışları</CardTitle>
                  <Settings className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">{connectedInstances.length}</div>
                  <p className="text-xs text-muted-foreground">Oluşturulan YZ iş akışı</p>
                </CardContent>
              </Card>
              ...
              <div className="lg:col-span-1">
                {packageInfo && (
                  <PackageManagement language={language} packageInfo={packageInfo} onPackageChange={setPackageInfo} />
                )}
              </div>
            </div>
          )}
            ...
            <Card className="hologram-card border-0 bg-gradient-to-r from-green-50/10 to-emerald-50/10 backdrop-blur-sm shadow-2xl animate-fade-in">
              <CardHeader className="pb-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div>
                    <CardTitle className="text-xl sm:text-2xl font-bold neon-text text-balance">
                      Bağlı Bot'larınız
                    </CardTitle>
                    <CardDescription className="text-neon-cyan font-medium text-base sm:text-lg">
                      {connectedInstances.length} WhatsApp bot'u aktif
                    </CardDescription>
                  </div>
                  <Button
                    onClick={handleCreateNewInstance}
                    disabled={packageInfo?.remaining_instances === 0}
                    className="tech-button gap-2 px-4 sm:px-6 py-2 sm:py-3 text-white font-bold shadow-2xl shadow-neon-blue/30 w-full sm:w-auto"
                  >
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <TabsList className="hologram-card grid w-full sm:max-w-3xl grid-cols-3 h-12 sm:h-14 bg-background/50 backdrop-blur-sm border-0 shadow-lg">
                        ...
                        >
                          ...
                          <span className="hidden xs:inline">Kontrol</span>
                      ...
                      <Badge
                        variant="outline"
                        className="hologram-card gap-2 px-3 sm:px-4 py-2 font-medium border-neon-cyan/30 text-neon-cyan bg-background/50 backdrop-blur-sm text-xs sm:text-sm w-full sm:w-auto justify-center sm:justify-start"
                      >
