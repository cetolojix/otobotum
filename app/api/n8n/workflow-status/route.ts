    ...
    if (workflowId) {
      ...
      const workflows = (data.data || []).filter((workflow: any) => {
        ...
        const isMatch =
          ...
          workflowName.includes(`whatsapp bot - ${instance}`) ||
          workflowName.includes(`whatsapp ai - ${instance}`) ||
          workflowName.includes(`bot - ${instance}`) ||
          workflowName.includes(`ai - ${instance}`) ||
          // Exact match for instance name after dash or space
          workflowName.match(new RegExp(`[\\s\\-_]${instance.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i"))
