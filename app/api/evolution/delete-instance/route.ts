...
async function deleteInstanceFromDatabase(instanceName: string) {
  try {
    console.log("[v0] Deleting instance from database:", instanceName)

    const { error } = await supabase.from("instances").delete().eq("instance_name", instanceName)

    if (error) {
      console.error("[v0] Failed to delete instance from database:", error)
      throw error
    }

    console.log("[v0] Instance deleted from database successfully:", instanceName)
  } catch (error) {
    console.error("[v0] Error deleting instance from database:", error)
    throw error
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (response.ok || response.status === 404) {
      ...
      await deleteInstanceFromDatabase(instanceName)
