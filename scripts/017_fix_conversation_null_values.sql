-- Fix toggle_conversation_ai function to properly handle customer data
-- The issue was that customer_phone and customer_name were being stored as NULL
...
BEGIN
  ...
  WHERE COALESCE(c.customer_phone, '') = COALESCE(p_customer_phone, '')
    AND c.instance_id = v_instance_id
  LIMIT 1;

  -- If conversation doesn't exist, create it
  IF v_conversation_id IS NULL THEN
    -- Explicitly cast TEXT to CHARACTER VARYING for customer_phone
    INSERT INTO conversations (
      ...
      v_instance_id,
      p_customer_phone::CHARACTER VARYING,  -- Explicit cast
      p_customer_name,
      p_instance_name,
      p_ai_enabled,
  ...
  ELSE
    ...
    SET 
      ...
      customer_name = COALESCE(p_customer_name, customer_name),  -- Keep existing if new is NULL
      updated_at = NOW()
